...
export function Header() {
  ...
  let gradeName = t('gradeDebutant')
  let gradeColor = "text-zinc-500"

  if (progress.level >= 11 && progress.level <= 20) { GradeIcon = Medal; gradeName = t('gradeIntermediaire'); gradeColor = "text-zinc-400" }
  else if (progress.level >= 21 && progress.level <= 30) { GradeIcon = Trophy; gradeName = t('gradeAvance'); gradeColor = "text-zinc-300" }
  else if (progress.level >= 31 && progress.level <= 45) { GradeIcon = Star; gradeName = t('gradeElite'); gradeColor = "text-zinc-100" }
  else if (progress.level > 45) { GradeIcon = Crown; gradeName = t('gradeLegende'); gradeColor = "text-white" }
        ...
        <div className="flex items-center gap-6">
          <div className="hidden sm:flex flex-col items-end gap-1.5 w-32 md:w-48">
            <div className="flex justify-between w-full items-end">
              <span className="text-[10px] font-bold uppercase tracking-widest text-white">{t('niveauCourt')} {progress.level}</span>
