...
export default function SessionForm({ dateActive, isRestDayMode, setIsRestDayMode, pasDuJour }: Props) {
  ...
  const handleAIGeneration = async () => {
    ...
    try {
      ...
      const newExercices: ExerciceRow[] = aiData.map((ex: any) => {
        ...
        return { id: null, name: ex.name, comments: ex.comments || '', coachTracking, tracking };
      });
  ...
  const validerMission = async () => {
    ...
    } catch (e: any) { alert("Erreur lors de la validation : " + e.message); } finally { setIsValidating(false); }
  }
  ...
  const updateExerciceCommentaire = (index: number, valeur: string) => {
    const nouvelleListe = [...exercices]; nouvelleListe[index].comments = valeur; setExercices(nouvelleListe)
  }
  const ajouterSerieCoach = (exIndex: number) => {
    const nouvelleListe = [...exercices]; nouvelleListe[exIndex].coachTracking.push({ reps: '', weight: '', rpe: '' }); nouvelleListe[exIndex].tracking.push({ reps: '', weight: '', rpe: '' }); setExercices(nouvelleListe)
  }
  const supprimerSerieCoach = (exIndex: number, setIndex: number) => {
    const nouvelleListe = [...exercices]; nouvelleListe[exIndex].coachTracking.splice(setIndex, 1);
  ...
  const updateSerieCoach = (exIndex: number, setIndex: number, champ: keyof SetData, valeur: string) => {
    const nouvelleListe = [...exercices]; nouvelleListe[exIndex].coachTracking[setIndex] = { ...nouvelleListe[exIndex].coachTracking[setIndex], [champ]: valeur }; setExercices(nouvelleListe)
  }
  const ajouterSerieAthlete = (exIndex: number) => {
    const nouvelleListe = [...exercices]; nouvelleListe[exIndex].tracking.push({ reps: '', weight: '', rpe: '' }); setExercices(nouvelleListe)
  }
  const supprimerSerieAthlete = (exIndex: number, setIndex: number) => {
    const nouvelleListe = [...exercices]; nouvelleListe[exIndex].tracking.splice(setIndex, 1); setExercices(nouvelleListe)
  }
  const updateSerieAthlete = (exIndex: number, setIndex: number, champ: keyof SetData, valeur: string) => {
    const nouvelleListe = [...exercices]; nouvelleListe[exIndex].tracking[setIndex] = { ...nouvelleListe[exIndex].tracking[setIndex], [champ]: valeur }; setExercices(nouvelleListe)
  }
