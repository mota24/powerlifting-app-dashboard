...
import { Target, Activity, Check, Moon, Footprints, Battery, Coffee, Plus, Trash2, MessageSquare } from 'lucide-react'
...
export default function SessionForm({ dateActive }: Props) {
  ...
  // --- FILTRE INTELLIGENT DES EXERCICES SELON LE JOUR ---
  const getExercicesDuJour = () => {
    switch(jourSemaine) {
      case 1: return [...LIFT_SQUAT, ...LIFT_BENCH, ...ACCESSORIES] // Lundi
      case 2: return [...LIFT_BENCH, ...LIFT_DEADLIFT, ...ACCESSORIES] // Mardi
      case 3: return [...LIFT_SQUAT, ...LIFT_BENCH, ...ACCESSORIES] // Mercredi
      case 4: return [...LIFT_BENCH, ...ACCESSORIES] // Jeudi
      case 6: return [...LIFT_SQUAT, ...LIFT_BENCH, ...LIFT_DEADLIFT] // Samedi SBD (Pas d'accessoires par défaut)
    ...
    const chargerSeance = async () => {
      ...
      if (data && data.length > 0) {
        const listeExercices = data.map(item => ({
          ...
          athleteSets: item.athlete_sets?.toString() || '',
          athleteReps: item.athlete_reps?.toString() || '',
          athleteWeight: item.athlete_weight?.toString() || '',
          athleteRpe: item.athlete_rpe?.toString() || '',
          comments: item.comments || '', // Récupération du commentaire
        }))
  ...
  const creerExerciceVierge = (): ExerciceRow => ({
    ...
    athleteSets: '', athleteReps: '', athleteWeight: '', athleteRpe: '',
    comments: '' // Commentaire vierge
  })
  ...
  const updateExercice = (index: number, champ: keyof ExerciceRow, valeur: string) => {
    const nouvelleListe = [...exercices]
    nouvelleListe[index][champ] = valeur
    setExercices(nouvelleListe)
  }
  ...
  const handleSave = async () => {
    ...
    const promesses = exercices.map(ex => {
      const payload = {
        ...
        athlete_sets: ex.athleteSets ? parseInt(ex.athleteSets) : null,
        athlete_reps: ex.athleteReps ? parseInt(ex.athleteReps) : null,
        athlete_weight: ex.athleteWeight ? parseFloat(ex.athleteWeight) : null,
        athlete_rpe: ex.athleteRpe ? parseFloat(ex.athleteRpe) : null,
        comments: ex.comments || null, // Envoi du commentaire
    ...
    <div className="space-y-6 animate-in fade-in pb-10">
      ...
      <div className="space-y-6">
        {exercices.map((ex, index) => (
          <div key={index} className="p-4 rounded-xl border border-slate-800 bg-slate-900/50 space-y-4 relative group">
            
            <div className="flex items-center gap-3">
              <div className="bg-slate-800 text-slate-400 px-3 py-1 rounded-md text-sm font-bold">{index + 1}</div>
              
              {/* INPUT AVEC AUTO-COMPLÉTION (DATALIST) */}
              <input 
                list={`liste-exos-${jourSemaine}`}
                placeholder="Nom de l'exercice (clique pour la liste)" 
              ...
              <datalist id={`liste-exos-${jourSemaine}`}>
                {suggestionsDuJour.map(nomExo => (
                  <option key={nomExo} value={nomExo} />
                ))}
              </datalist>

              <button onClick={() => supprimerExercice(index, ex.id)} className="p-2 text-slate-500 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="p-3 rounded-xl border border-slate-800 bg-slate-900/30">
                <h3 className="text-xs font-bold text-slate-400 mb-3 flex items-center gap-2 uppercase tracking-wider"><Target className="size-3" /> Prévu</h3>
                <div className="grid grid-cols-4 gap-2">
                  <div><label className="text-[10px] text-slate-500 uppercase">Séries</label><input type="number" value={ex.coachSets} onChange={(e) => updateExercice(index, 'coachSets', e.target.value)} className="w-full p-2 bg-slate-950 border border-slate-800 rounded-md text-slate-300 text-center outline-none" /></div>
                  <div><label className="text-[10px] text-slate-500 uppercase">Reps</label><input type="number" value={ex.coachReps} onChange={(e) => updateExercice(index, 'coachReps', e.target.value)} className="w-full p-2 bg-slate-950 border border-slate-800 rounded-md text-slate-300 text-center outline-none" /></div>
                  <div><label className="text-[10px] text-slate-500 uppercase">Poids</label><input type="number" value={ex.coachWeight} onChange={(e) => updateExercice(index, 'coachWeight', e.target.value)} className="w-full p-2 bg-slate-950 border border-slate-800 rounded-md text-slate-300 text-center outline-none" /></div>
                  <div><label className="text-[10px] text-slate-500 uppercase">RPE</label><input type="number" value={ex.coachRpe} onChange={(e) => updateExercice(index, 'coachRpe', e.target.value)} className="w-full p-2 bg-slate-950 border border-slate-800 rounded-md text-slate-300 text-center outline-none" /></div>
                </div>
              </div>

              <div className="p-3 rounded-xl border border-blue-500/30 bg-blue-500/5">
                <h3 className="text-xs font-bold text-blue-400 mb-3 flex items-center gap-2 uppercase tracking-wider"><Check className="size-3" /> Réalisé</h3>
                <div className="grid grid-cols-4 gap-2">
                  <div><label className="text-[10px] text-blue-500/70 uppercase">Séries</label><input type="number" value={ex.athleteSets} onChange={(e) => updateExercice(index, 'athleteSets', e.target.value)} className="w-full p-2 bg-slate-950 border border-blue-500/30 rounded-md text-white text-center outline-none focus:border-blue-500" /></div>
                  <div><label className="text-[10px] text-blue-500/70 uppercase">Reps</label><input type="number" value={ex.athleteReps} onChange={(e) => updateExercice(index, 'athleteReps', e.target.value)} className="w-full p-2 bg-slate-950 border border-blue-500/30 rounded-md text-white text-center outline-none focus:border-blue-500" /></div>
                  <div><label className="text-[10px] text-blue-500/70 uppercase">Poids</label><input type="number" value={ex.athleteWeight} onChange={(e) => updateExercice(index, 'athleteWeight', e.target.value)} className="w-full p-2 bg-slate-950 border border-blue-500/30 rounded-md text-white text-center outline-none focus:border-blue-500" /></div>
                  <div><label className="text-[10px] text-blue-500/70 uppercase">RPE</label><input type="number" step="0.5" value={ex.athleteRpe} onChange={(e) => updateExercice(index, 'athleteRpe', e.target.value)} className="w-full p-2 bg-slate-950 border border-blue-500/30 rounded-md text-white text-center outline-none focus:border-blue-500" /></div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-2 pt-3 border-t border-slate-800/50">
              ...
              <input 
                placeholder="Ex: Tempo 3-1-0, douleur épaule, facile..." 
                value={ex.comments} 
                onChange={(e) => updateExercice(index, 'comments', e.target.value)} 
