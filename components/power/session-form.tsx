...
export default function SessionForm({ dateActive }: Props) {
  ...
  const creerExerciceVierge = (): ExerciceRow => ({
    id: null, name: '', coachTracking: [{ reps: '', weight: '', rpe: '' }], tracking: [{ reps: '', weight: '', rpe: '' }], comments: '' 
  })
    ...
    return (
      <div className="space-y-6 animate-in fade-in pb-10">
        <div className="p-8 rounded-xl border border-slate-800 bg-slate-900/30 flex flex-col items-center text-center space-y-4">
          ...
          <div><h2 className="text-lg font-bold text-slate-200">Jour de Repos</h2></div>
        </div>
        <div className="p-4 rounded-xl border border-slate-800 bg-slate-900/50">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center justify-between p-3 bg-slate-950 rounded-lg border border-slate-800"><input type="range" min="1" max="10" value={fatigue} onChange={(e) => setFatigue(parseInt(e.target.value))} className="w-full accent-red-500" /></div>
            <div className="flex items-center justify-between p-3 bg-slate-950 rounded-lg border border-slate-800"><input type="number" step="0.5" value={sommeil} onChange={(e) => setSommeil(parseFloat(e.target.value))} className="w-16 bg-transparent text-lg font-bold text-white outline-none" /></div>
            <div className="flex items-center justify-between p-3 bg-slate-950 rounded-lg border border-slate-800"><input type="number" value={pas} onChange={(e) => setPas(parseInt(e.target.value))} className="w-full bg-transparent text-lg font-bold text-white outline-none" /></div>
          </div>
        </div>
        <button onClick={handleSaveMetricsOnly} disabled={loading} className="w-full p-4 rounded-xl font-bold bg-blue-600 hover:bg-blue-700 text-white">Enregistrer le suivi</button>
      </div>
    )
  }
      ...
      {exercices.map((ex, exIndex) => (
         <div key={exIndex} className="p-4 rounded-xl border border-slate-800 bg-slate-900/50 space-y-4 relative group shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <input list={`liste-exos-${jourSemaine}`} placeholder="Nom de l'exercice..." className="flex-1 p-2 bg-slate-950 border border-slate-800 rounded-lg text-white outline-none" value={ex.name} onChange={(e) => updateExerciceNom(exIndex, e.target.value)} />
              <datalist id={`liste-exos-${jourSemaine}`}>{getExercicesDuJour().map(nomExo => <option key={nomExo} value={nomExo} />)}</datalist>
              <button onClick={() => supprimerExercice(exIndex, ex.id)} className="p-2 text-slate-500 hover:text-red-500"><Trash2 className="size-5" /></button>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="p-3 rounded-xl border border-slate-800 bg-slate-900/30 flex flex-col h-full">
                <h3 className="text-xs font-bold text-slate-400 mb-3 uppercase tracking-wider"><Target className="size-3 inline mr-2" /> Prescription</h3>
                {ex.coachTracking.map((set, setIndex) => (
                   <div key={setIndex} className="grid grid-cols-[auto_1fr_1fr_1fr_auto] gap-2 items-center mb-2">
                     <span className="w-6 text-xs text-slate-600">S{setIndex + 1}</span>
                     <input type="number" value={set.reps} onChange={(e) => updateSerieCoach(exIndex, setIndex, 'reps', e.target.value)} className="w-full p-2 bg-slate-950 border rounded-md text-center" />
                     <input type="number" value={set.weight} onChange={(e) => updateSerieCoach(exIndex, setIndex, 'weight', e.target.value)} className="w-full p-2 bg-slate-950 border rounded-md text-center" />
                     <input type="number" step="0.5" value={set.rpe} onChange={(e) => updateSerieCoach(exIndex, setIndex, 'rpe', e.target.value)} className="w-full p-2 bg-slate-950 border rounded-md text-center" />
                     <button onClick={() => supprimerSerieCoach(exIndex, setIndex)}><X className="size-4" /></button>
                   </div>
                ))}
                <button onClick={() => ajouterSerieCoach(exIndex)} className="mt-2 w-full text-xs font-bold text-slate-400">Ajouter série</button>
              </div>
              <div className="p-3 rounded-xl border border-blue-500/30 bg-blue-500/5 flex flex-col h-full">
                <h3 className="text-xs font-bold text-blue-400 mb-3 uppercase tracking-wider"><Check className="size-3 inline mr-2" /> Validé</h3>
                {ex.tracking.map((set, setIndex) => (
                   <div key={setIndex} className="grid grid-cols-[auto_1fr_1fr_1fr_auto] gap-2 items-center mb-2">
                     <span className="w-6 text-xs text-slate-500">S{setIndex + 1}</span>
                     <input type="number" value={set.reps} onChange={(e) => updateSerieAthlete(exIndex, setIndex, 'reps', e.target.value)} className="w-full p-2 bg-slate-950 border border-blue-500/30 rounded-md text-white text-center" />
                     <input type="number" value={set.weight} onChange={(e) => updateSerieAthlete(exIndex, setIndex, 'weight', e.target.value)} className="w-full p-2 bg-slate-950 border border-blue-500/30 rounded-md text-white text-center" />
                     <input type="number" step="0.5" value={set.rpe} onChange={(e) => updateSerieAthlete(exIndex, setIndex, 'rpe', e.target.value)} className="w-full p-2 bg-slate-950 border border-blue-500/30 rounded-md text-white text-center" />
                     <button onClick={() => supprimerSerieAthlete(exIndex, setIndex)}><X className="size-4" /></button>
                   </div>
                ))}
                <button onClick={() => ajouterSerieAthlete(exIndex)} className="mt-2 w-full text-xs font-bold text-blue-400">Ajouter série</button>
              </div>
            </div>
            <input placeholder="Notes..." value={ex.comments} onChange={(e) => updateExerciceCommentaire(exIndex, e.target.value)} className="w-full p-2 bg-transparent border border-slate-800 rounded-md text-sm text-slate-300" />
         </div>
      ))}
      <button onClick={ajouterExercice} className="w-full py-3 border-2 border-dashed border-slate-700 hover:border-blue-500 rounded-xl">Ajouter un exercice</button>
      
      <button onClick={propagerSemaine1VersBloc} className="w-full py-3 bg-slate-800 hover:bg-slate-700 rounded-xl font-bold flex items-center justify-center gap-2 border border-slate-700">
        <Copy className="size-5" /> Propager la semaine sur le Bloc
      </button>

      <button onClick={handleSave} disabled={loading} className={cn("w-full p-4 rounded-xl font-bold transition-all", saved ? "bg-emerald-600 text-white" : "bg-blue-600 text-white")}>
        {loading ? 'Enregistrement...' : saved ? 'Séance Mémorisée !' : 'Enregistrer la séance'}
      </button>
    </div>
  )
}