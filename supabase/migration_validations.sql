...
--
-- Deux problèmes :
--   1. On pouvait valider une séance des jours après pour gagner des points
--      et prolonger sa série.
--   2. Le classement comptait une séance dès qu'une série était notée, même
--      si « Terminer la séance » n'avait jamais été validé.
--
-- L'app ne gardait que la date de la DERNIÈRE validation : impossible de
-- savoir quels jours ont été validés. On journalise désormais chaque
-- validation, et la règle « le jour même » est garantie par la base :
-- même en contournant l'interface, une validation d'un autre jour est refusée.
--
-- Une validation ne se modifie ni ne s'efface (aucune policy UPDATE/DELETE).
--
...
ALTER TABLE public.validations_seance ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "validations_select" ON public.validations_seance;
...
REVOKE ALL ON public.validations_seance FROM anon;


-- ────────────────────────────────────────────────────────────
-- 2. CLASSEMENT — une séance ne compte que si elle a été validée
--    le jour même ET contient au moins une série notée.
--
-- Barème — source de vérité unique, l'app ne recalcule rien :
--   8 000 pas ou plus dans la journée ............ +10 par jour
--   Séance validée le jour même (série notée) .... +20 par jour
--   Toutes les séances prévues validées .......... +30
--       prévue = une prescription existe ce jour-là ; si la semaine
--       n'en contient aucune, l'objectif est 3 séances validées
--   Jours actifs d'affilée dans la semaine ........ +5 par jour, max +35
--       actif = séance validée OU 8 000 pas
-- ────────────────────────────────────────────────────────────
...
-- d'ignorer la nouvelle table et la fonction recréée.
...
--   policies = validations_insert_jour_meme:INSERT, validations_select:SELECT
--   classement_security_definer = true, anon_peut_executer = false
-- ============================================================
SELECT
  (SELECT c.relrowsecurity FROM pg_class c WHERE c.oid = 'public.validations_seance'::regclass) AS rls_active,
  (SELECT string_agg(p.policyname || ':' || p.cmd, ', ' ORDER BY p.policyname)
     FROM pg_policies p
     WHERE p.schemaname = 'public' AND p.tablename = 'validations_seance') AS policies,
  (SELECT f.prosecdef
     FROM pg_proc f JOIN pg_namespace n ON n.oid = f.pronamespace
     WHERE n.nspname = 'public' AND f.proname = 'classement_semaines') AS classement_security_definer,
  has_function_privilege('anon', 'public.classement_semaines(integer)', 'EXECUTE') AS anon_peut_executer;
