...
-- ============================================================
-- CLASSEMENT AU MOIS — à coller dans Supabase > SQL Editor.
--
-- Le classement se joue désormais sur le mois et repart de zéro au mois
-- suivant ; l'app garde la liste des mois passés avec leur vainqueur.
--
-- Trois règles à retenir :
--   1. Le comptage démarre le 14/09/2026. La semaine du 7 n'entre pas dans
--      septembre : le second compte n'avait l'app que depuis le jeudi.
--   2. Un jour sans aucune séance prévue compte comme un jour de repos : il
--      ne casse plus la série.
--   3. L'objectif reste hebdomadaire (3 séances validées dans une semaine)
--      mais rapporte +30 pour CHAQUE semaine réussie du mois.
--
-- Barème : 10 points par jour à 8 000 pas, 20 par séance validée,
-- 30 par semaine à 3 séances, 5 par jour de série (35 au plus).
-- Idempotent : relançable sans risque. Ne modifie aucune donnée.
-- L'ancienne fonction classement_semaines reste en place mais n'est plus
-- utilisée par l'app ; elle pourra être supprimée plus tard.
-- ============================================================
...
AS $$
  ...
  -- Jour où une séance était prévue : une ligne hors repos portant un chiffre,
  -- dans le plan du coach ou dans le réalisé.
  prevus AS (
  ...
  -- Série : jours actifs consécutifs. Un jour sans séance prévue compte comme
  -- un jour de repos. Soustraire le rang à la date donne la même valeur à tous
  -- les jours d'une même suite ininterrompue.
  ilots AS (
    SELECT
      dt.debut,
      dt.user_id,
  ...
  SELECT
    ...
    (sc.nb_8000 * 10 + sc.nb_seances * 20 + sc.obj_ok * 30 + least(sc.nb_serie, 7) * 5)::integer,
...
-- ============================================================
-- VÉRIFICATION — une ligne par joueur pour le mois en cours.
-- Attendu en septembre 2026 : depuis = 2026-09-14 (la semaine du 7 n'est
-- pas comptée) et points = pts_pas + pts_seances + pts_objectif + pts_serie.
-- ============================================================
SELECT mois, depuis, prenom, points, pts_pas, pts_seances, pts_objectif, pts_serie,
       jours_8000, seances, objectif_fait, objectif_semaines, serie
FROM public.classement_mois(1)
